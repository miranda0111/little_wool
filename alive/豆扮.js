...
[MITM]
hostname = api.imdouban.com

api.imdouban.com/v1/users url script-request-header https://kireinasakura.coding.net/p/could/d/there/git/raw/master/db.js
 ...
 class UserInfo
 {
 	...
 	async kong()
 	{
 		try
 		{
 			...
 			await this.label()
 			console.log(`账号[${this.name}] ` + this.nickname + ` 资料设置为 ` + result.data.reside)
 		}
 		catch (e)
 		{
 			console.log(e)
 	...
 	async receive()
 	{
 		try
 		{
 			...
 			if(this.code == 200)
 			{
 				console.log(`\n账号[${this.name}] ` + this.nickname + ` 提现 :` + this.message)
 			}
 	...
 	async view()
 	{
 		try
 		{
 			...
 			{
 				console.log(`账号[${this.name}] ` + this.nickname + ` \n绑定支付宝:` + this.zfb + ` 可提金额:` + this.money + `\n`)
 				await this.receive()
 			}
