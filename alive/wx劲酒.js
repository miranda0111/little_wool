 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export jingjiu_data=' authorization '   ,多账号用 换行 或 @ 分割
 * 抓jjw.jingjiu.com域名下authorization的值填入变量
 */
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.jingjiu_data : $.getdata('jingjiu_data')) || '';
 let msg, ck;
 let ck_status = true;
 let host = 'jjw.jingjiu.com';
 ...
 !(async () => {
	 let ckArr = await checkEnv(ckStr, "jingjiu_data");
	 await tips(ckArr);
 ...
 async function signin() {
	 try {
		 let url = {
			 url: `${hostname}/app-jingyoujia/app/jingyoujia/taskContinuousRecord`,
			 headers: {
				 "Host": host,
				 "Content-Length": "12",
		 ...
		 console.log(result);
		 if (result?.code == 200) {
			 DoubleLog(`签到:${result?.msg} 🎉,签到获得:${result?.data.integral}积分 `);
			 await wait(3);
 ...
 async function jifen() {
	 try {
		 let url = {
			 url: `${hostname}/app-jingyoujia/app/jingyoujia/customer/queryCustIntegral`,
			 headers: {
				 "Host": host,
		 ...
		 console.log(result);
		 if (result?.code == 200) {
			 DoubleLog(`积分查询:${result?.msg} 🎉,当前积分:${result?.data} `);
			 await wait(3);
		 } else {
