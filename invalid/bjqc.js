 ...
 * ========= 青龙 =========
 * 地址:Bjev.com
 * 变量格式：export datafile = '请求头'
 * 变量格式：export bj_Authorization=' Bearer xxx @  Bearer xxx '  多个账号用 @分割 
 */
 ...
 //////////////////////
 let datafile = process.env.datafile;               // 这里是 从青龙的 配置文件 读取你写的变量
 let jiaocheng_dataArr = [];
 let bj_Authorization=[];
 let data = '';
 let msg = '';
 ...
 !(async () => {
     ...
     if (!(await Envs()))  	//多账号分割 判断变量是否为空  初步处理多账号
         ...
         console.log(`新手教程尝试，切勿使用)`);       // console.log是输出信息的，可以在脚本日志中看到输出（打印）的信息
         ...
         console.log(`\n=================== 共找到 ${bj_Authorization.length} 个账号 ===================`)
         ...
         for (let index = 0; index < bj_Authorization.length; index++) {
 
 
             let num = index + 1
             console.log(`\n========= 开始【第 ${num} 个账号】=========\n`)
 
             data = bj_Authorization[index].split('&');      // 这里是分割你每个账号的每个小项   
 ...
 function signin(timeout = 3 * 1000) {
     return new Promise((resolve) => {
         let url = {
             url: `http://api.smartservice.bjev.com.cn/gateway-api/v1/sign?id=${data[1]}`,    // 这是请求的 url 可以直接用我们抓包、精简后的URL
             headers: {            // headers 是请求体  可以直接用精简后的 hd  也就是服务器校验的部分，他需要啥，我们就给他啥  
                 ...
                 "Referer": " http://wx.smartservice.bjev.com.cn/",
 ...
 async function Envs() {
     if (bj_Authorization=='') {
         if (bj_Authorization.indexOf("@") != -1) {
            bj_Authorization.split("@").forEach((item) => {
                bj_Authorization.push(item);
             });
         } else {
            bj_Authorization.push(bj_Authorization);
         }
     } else {
         console.log(`\n 【${$.name}】：未填写变量 bj_Authorization`)
         return;
     }
